	...
	<?php
		...
		if (isset($_POST['Submit'])) {
			...
			if ($result->num_rows > 0) {
				...
				echo "<div style=\"background-color:transparent\" id='calendar' value='$del' ></div>";
//				echo "<div style=\"background-color:transparent\" id='AddanEvent' value='$del'><div>";
				echo "<button style=\"background-color:transparent\" id='print' onclick=\"printPage()\">Print Schedule</button>";
				//echo "<script> addEvents($del); </script>";
			} else {
            ...
            $(function() {
				...
				*/
				...
				$(document).click(function(event) { 
					if (!$(event.target).closest('.sb_dropdown').length &&
					   (!$(event.target).is('.sb_dropdown')) || (!$(event.target).is('#search-textbox'))) {
						if ($('.sb_dropdown').is(":visible")) {
							$('.sb_dropdown').hide();
						}
					}        
				})*/
